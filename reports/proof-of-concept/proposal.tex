...
The other one is preprocessed replay data.
After we parsing those replay data, we can gather useful information from replay data in string format instead of binary format(which is inappropriate for document NoSQL database to store).
It is appropriate to store such documents of JSON format in document-oriented NoSQL database like MongoDB.
